...
| 价值点 | 说明 |
|--------|------|
| **能力扩展** | 用户可以添加自定义 Skills，扩展 PromptCraft 的专业领域 |
...
PromptCraft 的 Skills 格式将**完全兼容 Claude Agent Skills 标准**，这意味着：

1. **双向兼容**：
   - 在 PromptCraft 中创建的 Skills 可以直接在 Claude Code 中使用
   - Claude Code 社区的 Skills 可以导入 PromptCraft
...
集成 Claude Agent Skills 将使 PromptCraft：
